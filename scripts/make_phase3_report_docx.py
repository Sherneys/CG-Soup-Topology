...
COND_TH = {"C0": "C0 เบสไลน์", "C1": "C1 topological loss",
           ...
           "C7": "C7 สัญญาณรบกวน σ=0.5% (stress)",
           "C7h": "C7h สัญญาณรบกวน σ=1% (stress)",
...
P(doc, "ผลหลัก: loss ให้กำไรเชิงโทโพโลยีแท้ (ชนะทั้งเบสไลน์และตัวควบคุมที่ปรับขนาด gradient เท่ากัน) "
       "ทั้งกับโพรง H2 (ลด error 4.0–7.9 เท่า) และ — จุดที่ resampling ทำไม่ได้ — ห่วง H1 (2.3 เท่า "
       ...
       "(2.1–10.4 เท่า ที่ Chamfer ดีกว่าเบสไลน์ทุกชิ้น)")
...
for shape in order:
    ...
    if shape == "spot":                     # 3e shapes start here — one intro
        ...
        P(doc, "ทำซ้ำโปรโตคอลเดิมทุกอย่าง (C0/C1/C2 × 3 seeds, ρ, ramp, งบต่อคลาสเท่าเดิม, "
               ...
               "ดีกว่าเบสไลน์ทุกชิ้น (2.1–10.4 เท่า) และโหมดล้มเหลวของตัวควบคุมเกิดซ้ำเป๊ะ "
...
bullet(doc, "⑦ ทนต่อสัญญาณรบกวน (stress test C7/C7h — ตามข้อเสนอของอาจารย์): ใส่ Gaussian "
            "noise ลงบน point cloud ที่ loss มองเห็นทุก refresh (σ = 0.5%/1% ของเส้นทแยง ≈ "
            "0.5/1.0 เท่าของระยะห่างจุดสุ่มที่ M=2048) — ผลลดลงแบบค่อยเป็นค่อยไปและไม่เคยแย่กว่า "
            "เบสไลน์: ทอรัส 2.0×/1.6× (C1 = 2.3×), ทรงกลม 2.7×/1.9× (C1 = 4.0×) ที่ Chamfer "
            "เท่าเดิมหรือดีกว่า และ #sig ถูกต้องทุก run; ตลอด 2,400 refreshes ที่มี noise "
            "ไม่มี Gabriel failure แม้แต่ครั้งเดียว (กลไก detachment สำรองไม่เคยถูกใช้) และ "
            "recruitment รับมือ noise เอง (ห่วงวงที่สองจมใต้เกณฑ์ทุก refresh ที่ σ=1% แต่ถูก "
            "recruit ถึงทุกครั้ง)")
